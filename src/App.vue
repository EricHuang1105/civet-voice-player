<template>
  <div class="player-container">
    ...
    <div class="cover-wrapper" :class="{ 'is-playing': isPlaying }">
        ...
        <img src="/civet.webp" alt="精靈封面" @error="handleImageError" />
      </div>
    </div>
...
.player-container {
  ...
  background: linear-gradient(145deg, #f4e9de 0%, #d4b59e 50%, #6f4d38 100%);
...
.bgm-toggle-btn {
  ...
  height: 36px;
  border-radius: 50%;
  border: 1px solid rgba(54, 33, 20, 0.2);
...
.leave-btn {
  ...
  border: 1px solid rgba(54, 33, 20, 0.2); /* 👈 替換這裡的 rgba */
  color: #362114; /* 👈 替換這裡的 hex 色碼 */
...
.cover-wrapper {
  ...
  box-shadow: 0 12px 35px rgba(54, 33, 20, 0.25);
...
.progress-slider {
  ...
  height: 4px;
  border-radius: 2px;
  background-color: rgba(54, 33, 20, 0.15);
  outline: none;
  cursor: pointer; 
}
